...
##Różne przydatne zmienne i obiekty
world = World(mapp)
...
for i in range(20):  ##Dodawanie losowych pułapek
    x = randint(0, world.width)
    y = randint(0, world.height)
    if str(world.data[y][x]) == '1':
        world.data[y][x] = '1_1'

for i in world.data:
    for j in i:
        if j == 'E':
            EndCoordinates = [counterx, countery]
        counterx += 1
    countery +=1
    counterx = 0
del counterx
